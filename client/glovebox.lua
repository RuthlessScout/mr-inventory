...
RegisterNetEvent("ex-inventory_glovebox:setOwnedVehicle")
AddEventHandler("ex-inventory_glovebox:setOwnedVehicle", function(vehicle)
...
RegisterNetEvent("ex-inventory_glovebox:openGlovebox")
AddEventHandler("ex-inventory_glovebox:openGlovebox", function()
	openGlovebox()
end)
...
--	while true do
--		Wait(0)
--		if IsControlJustReleased(0, 170) and (GetGameTimer() - GUI.Time) > 1000 then
--			openGlovebox()
--			GUI.Time = GetGameTimer()
--		end
--	end
...
RegisterKeyMapping('glovebox', 'Жабка', 'keyboard', 'f3')
...
function OpenCoffresInventoryMenu(plate, max, myVeh)
	ESX.TriggerServerCallback("ex-inventory_glovebox:getInventoryV", function(inventory)
	text = _U("glovebox_info", plate, (inventory.weight / 100), (max / 100))
	data = {plate = plate, max = max, myVeh = myVeh, text = text}
	TriggerEvent("ex-inventory:openGloveboxInventory", data, inventory.blackMoney, inventory.cashMoney, inventory.items, inventory.weapons)
	end, plate)
end
