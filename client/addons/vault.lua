RegisterNetEvent("ex-inventory:openVaultInventory")
AddEventHandler("ex-inventory:openVaultInventory", function(data)
...
RegisterNUICallback("PutIntoVault", function(data, cb)
	...
	if type(data.number) == "number" and math.floor(data.number) == data.number then
		...
		if data.item.type == "item_weapon" then
			...
			TriggerServerEvent("ex-inventory:putItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
		else
...
RegisterNUICallback("TakeFromVault", function(data, cb)
	...
	if type(data.number) == "number" and math.floor(data.number) == data.number then
		...
		TriggerServerEvent("ex-inventory:getItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
	end
	Wait(0)
	refreshVaultInventory()
	Wait(0)
