...
RegisterNetEvent("ex-inventory_trunk:setOwnedVehicule")
AddEventHandler("ex-inventory_trunk:setOwnedVehicule", function(vehicle)
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
	ESX.TriggerServerCallback("ex-inventory_trunk:getInventoryV", function(inventory)
		text = _U("trunk_info", plate, (inventory.weight / 100), (max / 100))
		data = {plate = plate, max = max, myVeh = myVeh, text = text}
		TriggerEvent("ex-inventory:openTrunkInventory", data, inventory.blackMoney, inventory.cashMoney, inventory.items, inventory.weapons)
	end, plate)
end
