...
RegisterNetEvent('ex-inventory:itemPopUp')
AddEventHandler('ex-inventory:itemPopUp', function(weapon)
    ...
    TriggerEvent('ex-inventory:notification', weapon, _U("weapon_used"), 1, false)
end)

AddEventHandler('ex-inventory:addCurrentWeapon', function(waeponToAdd, attachments, addToSlote)
    currentWeapon = waeponToAdd
    currentWepAttachs = attachments.attach
    local doBreak = false
    if addToSlote then
        TriggerServerEvent('ex-inventory:slotPut', waeponToAdd)
    end
end)

RegisterNetEvent('ex-inventory:removeCurrentWeapon')
AddEventHandler('ex-inventory:removeCurrentWeapon', function()
...
RegisterNetEvent('ex-inventory:useAttach')
AddEventHandler('ex-inventory:useAttach', function(attach)
    ...
    if currentWeapon ~= nil then
        ...
        if weapons[tostring(hash)] ~= nil and weapons[tostring(hash)][attach] ~= nil and not hasAttach then
            ESX.TriggerServerCallback('ex-inventory:removeItem', function(cb)
                if cb then
	...
	function()
        if not IsEntityDead(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), true) and not removingAttach then
            if currentWeapon ~= nil then
                ...
                TriggerEvent("DoShortHudText", _U("no_gun_in_hand"), 2)
            end
		end
	end
)
    ...
    ESX.TriggerServerCallback('ex-inventory:doesWeaponHas', function(hasWeaponId)
        ...
        if checkh[weapon] == hash then
            if GetSelectedPedWeapon(playerPed) == hash then
                ESX.TriggerServerCallback('ex-inventory:addPlayerItem', function(cb)
                end, weapon, 1)
            end
        end
        ...
        canFire = true
        TriggerEvent('ex-inventory:notification', weapon, _U("weapon_pulled"), 1, false)
    end, hash)
end
...
function GiveWeapon(weapon)
    ...
    if weapon == 'WEAPON_PETROLCAN' then
      ...
      ESX.TriggerServerCallback('ex-inventory:getAmmoCount', function(gunInfo)
        ...
        if checkh[weapon] == hash then
            ESX.TriggerServerCallback('ex-inventory:takePlayerItem', function(cb)
...
function GenerateWeapon()
	...
	while true do
		...
		ESX.TriggerServerCallback('ex-inventory:isWeaponNumberTaken', function(isWeaponTaken)
