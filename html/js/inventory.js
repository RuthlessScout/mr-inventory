...
window.addEventListener("message", function (event) {
    ...
    //    $("#noSecondInventoryMessage").html(invLocale.secondInventoryNotAvailable);
        ...
        $.post("https://mr-inventory/GiveItem", JSON.stringify({
            player: player,
...
function closeInventory() {
    $(".ui").fadeOut();
    $.post("https://mr-inventory/NUIFocusOff", JSON.stringify({}));
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                $.post("https://mr-inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1,
                }));
            }
...
$(document).ready(function () {
        ...
        if (type === "normal" && itemInventory === "main" && itemData.usable) {
            if (itemData.name.includes("WEAPON_")) {
                if (itemInventory === "fast") 
                {
                    ...
                    $.post("https://mr-inventory/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
                
            } else {
                $.post("https://mr-inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
                closeInventory();
            }
    ...
    $(document).on('contextmenu', '.item', function (e) {
        ...
        if(e.shiftKey) {
            if (itemInventory === "second") {
                if (type === "trunk") {
                    $.post("https://mr-inventory/TakeFromTrunk", JSON.stringify({
    
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("https://mr-inventory/TakeFromProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("https://mr-inventory/TakeFromVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "locker") {
                    $.post("https://mr-inventory/TakeFromLocker", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("https://mr-inventory/TakeFromPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                    ...
                    $.post("https://mr-inventory/TakeFromShop", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motels") {
                    $.post("https://mr-inventory/TakeFromMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motelsbed") {
                    $.post("https://mr-inventory/TakeFromMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("https://mr-inventory/TakeFromGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "bag") {
                    $.post("https://mr-inventory/TakeFromBag", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
                ...
                if (type === "trunk") {
                    $.post("https://mr-inventory/PutIntoTrunk", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("https://mr-inventory/PutIntoProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("https://mr-inventory/PutIntoVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "locker") {
                    $.post("https://mr-inventory/PutIntoLocker", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("https://mr-inventory/PutIntoPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "motels") {
                    $.post("https://mr-inventory/PutIntoMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }))
                } else if (type === "motelsbed") {
                    $.post("https://mr-inventory/PutIntoMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("https://mr-inventory/PutIntoGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "bag") {
                    $.post("https://mr-inventory/PutIntoBag", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("https://mr-inventory/DropItem", JSON.stringify({
