...
window.addEventListener("message", function (event) {
    ...
    //    $("#noSecondInventoryMessage").html(invLocale.secondInventoryNotAvailable);
        ...
        $.post("https://its-inv/GiveItem", JSON.stringify({
            player: player,
...
function closeInventory() {
    $(".ui").fadeOut();
    $.post("https://its-inv/NUIFocusOff", JSON.stringify({}));
...
function makeDraggables(){ // Fast items by Condo, merged to the original ESX Inventory HUD by Tsrak.
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (itemData == undefined || itemData.usable == undefined || itemData.usable == false || itemData.name == 'cuffs' || itemData.name == 'blindfold') {
                return;
            }
            
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                $.post("https://its-inv/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1,
                }));
            }
...
$(document).ready(function () {
        ...
        if (type === "normal" && itemInventory === "main" && itemData.usable) {
            if (itemData.name.includes("WEAPON_")) {
                if (itemInventory === "fast") 
                {
                    ...
                    $.post("https://its-inv/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
                
            } else {
                $.post("https://its-inv/UseItem", JSON.stringify({
                    item: itemData
                }));
                closeInventory();
            }
    ...
    $(document).on('contextmenu', '.item', function (e) {
        ...
        if(e.shiftKey) {
            if (itemInventory === "second") {
                if (type === "trunk") {
                    $.post("https://its-inv/TakeFromTrunk", JSON.stringify({
    
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("https://its-inv/TakeFromProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("https://its-inv/TakeFromVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("https://its-inv/TakeFromPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                    ...
                    $.post("https://its-inv/TakeFromShop", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motels") {
                    $.post("https://its-inv/TakeFromMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motelsbed") {
                    $.post("https://its-inv/TakeFromMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("https://its-inv/TakeFromGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "bag") {
                    $.post("https://its-inv/TakeFromBag", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
                ...
                if (type === "trunk") {
                    $.post("https://its-inv/PutIntoTrunk", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("https://its-inv/PutIntoProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("https://its-inv/PutIntoVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("https://its-inv/PutIntoPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "motels") {
                    $.post("https://its-inv/PutIntoMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }))
                } else if (type === "motelsbed") {
                    $.post("https://its-inv/PutIntoMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("https://its-inv/PutIntoGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "bag") {
                    $.post("https://its-inv/PutIntoBag", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("https://its-inv/DropItem", JSON.stringify({
