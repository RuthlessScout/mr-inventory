...
AddEventHandler("ex-inventory_trunk:GetSharedDataStoreTrunk", function(plate, cb)
	cb(GetSharedDataStoreTrunk(plate))
end)

--[[RegisterServerEvent("ex-inventory_trunk:getOwnedVehicle")
AddEventHandler("ex-inventory_trunk:getOwnedVehicle", function()
	local vehicules = {}
	...
	function(result)
		...
		TriggerClientEvent("ex-inventory_trunk:setOwnedVehicule", _source, vehicules)
	end)
end)]]
...
function getTotalInventoryWeightTrunk(plate)
	local total
	TriggerEvent("ex-inventory_trunk:GetSharedDataStoreTrunk", plate, function(store)
...
ESX.RegisterServerCallback("ex-inventory_trunk:getInventoryV", function(source, cb, plate)
	TriggerEvent("ex-inventory_trunk:GetSharedDataStoreTrunk", plate, function(store)
		local blackMoney = 0
		local cashMoney = 0
		local items = {}
...
RegisterServerEvent("ex-inventory_trunk:getItem")
AddEventHandler("ex-inventory_trunk:getItem", function(plate, type, item, count, max, owned)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if type == "item_standard" then
		...
		if xPlayer.canCarryItem(item, count) then
			TriggerEvent("ex-inventory_trunk:GetSharedDataStoreTrunk", plate, function(store)
				...
				TriggerClientEvent("ex-inventory:refreshTrunkInventory", _source, data, blackMoney, cashMoney, items, weapons)
			end)
		else
...
RegisterServerEvent("ex-inventory_trunk:putItem")
AddEventHandler("ex-inventory_trunk:putItem", function(plate, type, item, count, max, owned, label)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
		...
		if (playerItemCount >= count and count > 0) then
			TriggerEvent("ex-inventory_trunk:GetSharedDataStoreTrunk", plate, function(store)
				...
				if (getTotalInventoryWeightTrunk(plate) + (getItemWeight(item) * count)) > max then
					TriggerClientEvent("DoLongHudText", source, _U("insufficient_space"), 1)
				else
					store.set("coffre", coffre)
					if item == 'WEAPON_PISTOL' or item == 'WEAPON_FLASHLIGHT' or item == 'WEAPON_STUNGUN' or item == 'WEAPON_KNIFE' 
					or item == 'WEAPON_BAT' or item == 'WEAPON_ADVANCEDRIFLE' or item == 'WEAPON_APPISTOL' or item == 'WEAPON_ASSAULTRIFLE'
...
ESX.RegisterServerCallback("ex-inventory_trunk:getPlayerInventory", function(source, cb)
