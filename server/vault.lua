RegisterServerEvent('ex-inventory:getItem')
AddEventHandler('ex-inventory:getItem', function(--[[owner,--]] job, type, item, count)
			...
			TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..job, function(inventory)
				...
				if count > 0 and inventoryItem.count >= count then
					if not xPlayer.canCarryItem(item, count) then
						TriggerClientEvent("DoLongHudText", _source, _U('player_cannot_hold'), 2)
					else
						inventory.removeItem(item, count)
						xPlayer.addInventoryItem(item, count)
						TriggerClientEvent("DoLongHudText", _source, _U('have_withdrawn', count, inventoryItem.label), 1)
					end
				else
					TriggerClientEvent("DoLongHudText", _source, _U('not_enough_in_vault'), 2)
				end
			end)
...
RegisterServerEvent('ex-inventory:putItem')
AddEventHandler('ex-inventory:putItem', function(--[[owner,--]] job, type, item, count)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
	local xPlayerOwner = ESX.GetPlayerFromIdentifier(xPlayer.identifier)
...
ESX.RegisterServerCallback('ex-inventory:getVaultInventory', function(source, cb, item, refresh)
