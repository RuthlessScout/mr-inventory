ESX.RegisterServerCallback('ex-inventory:removeItem', function(source,cb, item)
...
ESX.RegisterServerCallback('ex-inventory:giveWeapon', function(source, cb, targedId, item)
    local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('ex-inventory:changeWeaponOwner', function(originalOwner, ownerToGet, item)
    local hash = GetHashKey(item)
        ...
        if #results ~= 0 then
            MySQL.Async.execute('UPDATE ammunition SET `owner` = @owner WHERE `weapon_id` = @weapon_id', {
                ...
                if results2 then
                    ...
                    print("[ex-inventory] [^1ERROR^7] There was an error taking a weapon from dead person")
                end
            end)
        end
    end)
...
ESX.RegisterServerCallback('ex-inventory:doesWeaponHas', function(source,cb,hash)
    local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchAll('SELECT * FROM ammunition WHERE hash = @hash AND owner = @owner', {
        ['@hash'] = hash,
        ['@owner'] =  xPlayer.identifier
...
AddEventHandler('ex-inventory:weaponID', function(weaponID, identifier)
    MySQL.Async.execute('UPDATE ammunition SET `owner` = @owner WHERE `weapon_id` = @weapon_id', {
        ['@owner'] = identifier,
        ['@weapon_id'] = weaponID,
    }, function(results2)
        if results2 then
            ...
            print("[ex-inventory] [^1ERROR^7] There was an error picing a weapon from the ground")
        end
    end)
end)
...
Citizen.CreateThread(function()
	for i = 1, #Config.Attachments do
		ESX.RegisterUsableItem(Config.Attachments[i], function(source)
			TriggerClientEvent("ex-inventory:useAttach", source, Config.Attachments[i])
		end)
	end
end)
	...
	MySQL.Async.fetchAll('SELECT * FROM items WHERE LCASE(name) LIKE \'%weapon_%\'', {}, function(results)
		for k, v in pairs(results) do
			ESX.RegisterUsableItem(v.name, function(source)
				TriggerClientEvent('ex-inventory:itemPopUp', source, v.name)
			end)
		end
	end)
end)

RegisterServerEvent('ex-inventory:updateAmmoCount')
AddEventHandler('ex-inventory:updateAmmoCount', function(hash, wepInfo)
...
ESX.RegisterServerCallback('ex-inventory:isWeaponNumberTaken', function(source, cb, weapon)
...
ESX.RegisterServerCallback('ex-inventory:getAmmoCount', function(source, cb, hash)
	local player = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchAll('SELECT * FROM ammunition WHERE owner = @owner and hash = @hash', {
		['@owner'] = player.identifier,
		['@hash'] = hash
